...
-- e rode ./scripts/gerar-instalacao.sh de novo.
--
...
--    Troque pelo domínio real e, se quiser, aponte a unidade padrão.
--
--    insert into dominio_permitido (dominio, papel_padrao, unidade_id)
--    values ('seudominio.com.br', 'PROFESSOR', null);
--
-- 2. Crie seu usuário em Authentication → Users → Add user
--    (marque "Auto Confirm User" e defina uma senha).
--
-- 3. Promova-se a ADMIN, trocando o e-mail:
--
--    insert into pessoa_papel (pessoa_id, papel, unidade_id)
--    select p.id, 'ADMIN', u.id
--      from pessoa p cross join unidade u
--     where p.email = 'voce@seudominio.com.br'
--    on conflict do nothing;
--
-- 4. Confira o que foi criado:
--
--    select u.nome as unidade, count(distinct h.id) as horarios,
--           count(distinct po.id) as frotas
--      from unidade u
--      left join horario h on h.unidade_id = u.id
--      left join pool po on po.unidade_id = u.id
--     group by u.nome;
-- =============================================================================
